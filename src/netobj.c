...
void pok_network_object_load_module()
{
    treemap_init(&netobjs,(key_comparator)network_object_compar,NULL);
}
void pok_network_object_unload_module()
{
    treemap_delete(&netobjs);
}
uint32_t pok_network_object_allocate_unique_id()
{
    /* we never recycle ids; just get the next available one */
    return idtop++;
}
struct pok_netobj* pok_network_object_lookup(uint32_t id)
{
